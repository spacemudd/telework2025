...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">تعديل الخبرة</h1>

        <form action="{{ route('employee.experiences.update', ['locale' => app()->getLocale(), 'experience' => $experience->id]) }}" method="POST" class="bg-white border border-gray-200 rounded-lg p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="is_current" id="is_current" value="1" 
                           class="mr-2" {{ old('is_current', $experience->is_current) ? 'checked' : '' }}>
                    <span class="text-sm text-gray-700">أعمل حالياً في هذه الوظيفة</span>
                </label>
            </div>

            <div class="mb-4" id="end_date_container" style="{{ $experience->is_current ? 'display: none;' : '' }}">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="4" 
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $experience->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
