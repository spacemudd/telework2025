...
@section('admin-content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.job-postings.update', $jobPosting->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div id="existing_company_section">
                        ...
                        <select name="company_id" id="company_id"
                            ...
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" {{ old('company_id', $jobPosting->company_id) == $company->id ? 'selected' : '' }}>
                                    {{ $company->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-4">
                    <label for="sector_id" class="block text-sm font-medium text-gray-700 mb-1">
                        {{ __('words.sector') }}
                    </label>
                    <select name="sector_id" id="sector_id"
                        ...
                        @foreach($sectors as $sector)
                            <option value="{{ $sector->id }}" {{ old('sector_id', $jobPosting->sector_id) == $sector->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select name="employment_type" id="employment_type" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        <option value="full_time" {{ old('employment_type', $jobPosting->employment_type) == 'full_time' ? 'selected' : '' }}>
                            {{ __('words.full_time') }}
                        </option>
                        <option value="part_time" {{ old('employment_type', $jobPosting->employment_type) == 'part_time' ? 'selected' : '' }}>
                            {{ __('words.part_time') }}
                        </option>
                        <option value="contract" {{ old('employment_type', $jobPosting->employment_type) == 'contract' ? 'selected' : '' }}>
                            {{ __('words.contract') }}
                        </option>
                        <option value="freelance" {{ old('employment_type', $jobPosting->employment_type) == 'freelance' ? 'selected' : '' }}>
                            {{ __('words.freelance') }}
                        </option>
                        <option value="remote" {{ old('employment_type', $jobPosting->employment_type) == 'remote' ? 'selected' : '' }}>
                            {{ __('words.remote') }}
                        </option>
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="salary_min" id="salary_min" value="{{ old('salary_min', $jobPosting->salary_min) }}"
                               min="0" step="0.01"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        @error('salary_min')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="salary_max" id="salary_max" value="{{ old('salary_max', $jobPosting->salary_max) }}"
                               min="0" step="0.01"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        @error('salary_max')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="closing_date" id="closing_date" 
                           value="{{ old('closing_date', $jobPosting->closing_date ? $jobPosting->closing_date->format('Y-m-d') : '') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    @error('closing_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
